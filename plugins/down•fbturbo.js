...
async function fbDownloaderTurbo(url) {
  try {
    ...
    if (!videoUrl) throw new Error('تعذر العثور على رابط التحميل.')
...
let handler = async (m, { conn, text }) => {
  if (!text) return conn.reply(m.chat, `✳️ مثال:\n${m.prefix + m.command} https://www.facebook.com/...`, m, global.rcanal)
  
  await conn.reply(m.chat, "⏳ جاري جلب الفيديو... المرجو الانتظار قليلاً.", m, global.rcanal)
  
  try {
    const result = await fbDownloaderTurbo(text)
    
    // إرسال الفيديو مع القناة في الـ ContextInfo
    await conn.sendMessage(m.chat, { 
        video: { url: result.videoUrl }, 
        caption: `✅ تم تحميل الفيديو بنجاح (سيرفر تيربو)`,
        contextInfo: global.rcanal.contextInfo 
    }, { quoted: m })

  } catch (e) {
    await conn.reply(m.chat, "❌ فشل تحميل الفيديو، يرجى التأكد من الرابط أو المحاولة لاحقاً.", m, global.rcanal)
  }
}

handler.help = ['fbturbo']
handler.arabic = ['تيربوفيس']
handler.command = ['fbturbo','تيربوفيس']
