...
const {DisconnectReason, useMultiFileAuthState, MessageRetryMap, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, jidNormalizedUser, PHONENUMBER_MCC} = await import('@adiwajshing/baileys')
...
if (MethodMobile) throw new Error('You cannot use a pairing code with the mobile API')

let numeroTelefono
if (!!phoneNumber) {
numeroTelefono = phoneNumber.replace(/[^0-9]/g, '')
if (!Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
console.log(chalk.bgBlack(chalk.bold.greenBright(`🟣  Please enter your WhatsApp number.\n${chalk.bold.yellowBright("ADVICE: Copy the WhatsApp number and paste it into the console.")}\n${chalk.bold.yellowBright("Example: 2126××××××")}\n${chalk.bold.magentaBright('---> ')}`)))
process.exit(0)
}} else {
while (true) {
numeroTelefono = await question(chalk.bgBlack(chalk.bold.yellowBright('Please write your WhatsApp number.\nExample: 2126××××××\n')))
...
console.log(chalk.black(chalk.bgGreen(`👑 LINK CODE 👑`)), chalk.black(chalk.white(codigo)))
}, 3000)
}}
}
...
console.log(chalk.bold.cyanBright(`\n⚠️ NO CONNECTION, DELETE THE FOLDER ${global.sessions} AND SCAN THE QR  ⚠️`))
...
console.log(chalk.bold.yellowBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗\n┆ ⚠️ CONNECTION REPLACED, A NEW SESSION HAS BEEN OPENED. PLEASE CLOSE YOUR CURRENT SESSION FIRST..\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗`))
...
console.log(chalk.bold.redBright(`\n⚠️❗ REASON FOR DISCONNECTION UNKNOWN: ${reason || 'Not found'} >> ${connection || 'Not found'}`))
}}
}
